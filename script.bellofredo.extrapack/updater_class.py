...
class Updater:
	...
	def Update(self):
		...
		progressDialog.writeMsg(self.Language(30000).decode('utf-8'), "", "", 0)
		...
		for count, file in enumerate(self.UpdatableFiles):
			...
			while Flag:
				if activeCount() < 20:
					file = file.replace(self.SVNPathAddress,"")
					continueDownload = progressDialog.writeMsg(self.Language(30000).decode('utf-8'), self.Language(30004).decode('utf-8') +file.decode('utf-8'), "", count)
